'''
 Gaussian Process: no hyperparameter tuning neccessary ??
 
 start with:  "python scen_....py" 
...
#------------------------------------------------------------------------------
#n_days  = [1, 3, 5, 7, 10, 14]   # interval_length, intervals: 3,4,6
n_days  = [1, 7, 14]   # interval_length, intervals: 3,4,6
n_test  =  1

n_tsw = [ [1,1,1],  [2,1,1], [2,2,0], [3,1,0],  [2,2,2], [3,2,1], [4,1,1], [4,2,0], [5,1,0] ]

n_split  = 300# 1000 # 500  #100 #10  
n_sample =  10 # 30   # number of interval selection fullfilling the n_tsw conditions

name   = 'GP'          # name of saved figures
dirnam = 'GausProc/'    
name   = dirnam + name
...
kernel = Matern(length_scale=1.0, length_scale_bounds=(1e-01, 10.0), nu=1.5) 
model  = GaussianProcessRegressor(kernel, alpha=1.0, n_restarts_optimizer = 5, random_state = 1)
        ...
        for train, test in gss.split( X, y, groups=groups ):
            ...
            if( ( j < n_sample) and (nt == n_trans) and (ns == n_summer) and (nw == n_winter) ):
                ...
                for train_new, test_new in logo.split(X_new, y_new, gr_new):
                    ...
                    model.fit( X_train, y_train )  # internal split into train- and val-set
