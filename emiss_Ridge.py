'''
 ...
 Julian Adolphs, 18.11.2019
 Ridge Regression linear and with polynoms 
'''
...
name   = 'Ridge'    # name of saved figures
dirnam = 'Ridge/'    
name   = dirnam + name
...
regr = Ridge( solver='lsqr' )   # Regressor
...
for ntsw in n_tsw:     
    ...
    for nday in n_days: 
       ...
       #------------------------------------------------------------------------------
        ...
        p_dist = {"alpha": np.random.uniform(0, 2000, size=n_split*n_iter)}
     
        model = RandomizedSearchCV( regr, p_dist, n_iter, scoring='neg_mean_absolute_error', 
                                   n_jobs=None, cv=inner_cv, refit=True)
        ...
        for train, test in outer_cv.split( X, y, groups=groups ):
            ...
            if( ( j < n_sample) and (nt == n_trans) and (ns == n_summer) and (nw == n_winter) ):
                ...
                for train_new, test_new in inner_cv.split(X_new, y_new, gr_new):
                    ...
                    i = i + 1
                    #print(i, j, group_list, test_list, season_list, round(test_err, 3), round(pred_err, 3), round(ef_pred, 3), sep="   " )
        ...
        print(" ")
        #print("best_score (mean, std):   ", round(np.mean(best_scores), 3), round(np.std(best_scores),3), sep="  ")
        ...
        #==============================================================================
        # Re-Training without cv
        #==============================================================================
        
        best_scores, best_params, best_estims      = [], [], []
        pred_error, pred_emiss_all, pred_emiss_dis = [], [], []
        j = 0
